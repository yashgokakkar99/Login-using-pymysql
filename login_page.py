...
class login_page:
    def __init__(self, root):
        self.window = root
        self.window.title("Log In DYPCOE")
        # Set the window size
        ...
        self.frame1 = Frame(self.window, bg="magenta")
        self.frame1.place(x=0, y=0, width=450, relheight = 1)

        label1 = Label(self.frame1, text= "DYPCOE", font=("serif-sans", 30, "bold"), bg="magenta", fg="white").place(x=100,y=300)
        label2 = Label(self.frame1, text= "(Computer Engg Dept)", font=("times new roman", 20, "bold"), bg="magenta", fg="white").place(x=100,y=360)
        label3 = Label(self.frame1, text= "AKURDI PUNE", font=("serif-sans", 13, "bold"), bg="magenta", fg="white").place(x=100,y=420)

        #=============Entry Field & Buttons============

        self.frame2 = Frame(self.window, bg = "lightpink")
        self.frame2.place(x=450,y=0,relwidth=1, relheight=1)

        self.frame3 = Frame(self.frame2, bg="yellow")
        self.frame3.place(x=200,y=150,width=400,height=450)

        self.email_label = Label(self.frame3,text="Email Address", font=("times new roman",20,"bold"),bg="yellow", fg="black").place(x=50,y=40)
        ...
        self.password_label = Label(self.frame3,text="Password", font=("times new roman",20,"bold"),bg="yellow", fg="black").place(x=50,y=120)
        ...
        self.forgotten_pass = Button(self.frame3,text="Forgot password ?",command=self.forgot_func,font=("times new roman",15, "bold"),bd=0,cursor="hand2",bg="yellow",fg="blue").place(x=125,y=260,width=150)

        self.create_button = Button(self.frame3,text="Create New Account",command=self.redirect_window,font=("times new roman",18, "bold"),bd=0,cursor="hand2",bg="green",fg="white").place(x=80,y=320,width=250)

        self.delete_button = Button(self.frame3,text="Delete Account",command=self.del_acc,font=("times new roman",18, "bold"),bd=0,cursor="hand2",bg="red",fg="white").place(x=80,y=360,width=250)
        ...
        if self.email_entry.get() == "" or self.password_entry.get() == "" :
            ...
            try:
                ...
                if row == None:
                    ...
                    try:
                        cur.execute("delete from student_register where email=%s ", (self.email_entry.get()))
                        connection.commit()
